...
handlers._tokens.post = function(data, callback) {
    ...
    if(phone && password) {
        ...
        _performance.mark('begg user lookup');
        _data.read('users', phone, function(err, userData) {
            _performance.mark('user lookup complate');
            if(!err && userData) {
                // hash the sent password and compare to the password stored in user object
                _performance.mark('beg password hashing');
                const hashPassword = helpers.hash(password);
                ...
                if(hashPassword == userData.hashPassword) {
                    ...
                    _performance.mark('beg storing token');
                    _data.create('tokens', tokenId, tokenObject, function(err) {
                        ...
                        _performance.measure('user lookup', 'begg user lookup', 'user lookup complate');
                        _performance.measure('password hashing', 'beg password hashing', 'password hashing complete');
                        _performance.measure('token data creation', 'creating data for the token', 'beg storing token');
                        _performance.measure('token storing', 'beg storing token', 'storing token complete');
                ...
                _data.read('users', userPhone, function(err, userData) {
                    if(!err && userData) {
                        ...
                        if(userChecks.length < config.maxChecks) {
                            //verify that url given has DNS entry (and therefore can resolve)
                            ...
                            dns.resolve(hostName, function(err, records) {
                                if(!err && records) {
                                    ...
                                    callback(400, {'Error': 'the hostname of the url entered did not resolved to any DNS entry'});
                                }
                            });
                        } else {
